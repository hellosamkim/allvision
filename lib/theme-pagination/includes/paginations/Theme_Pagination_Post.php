<?php
/**
 * Theme Pagination - single post pagination class.
 * Provides the pagination for singular post.
 *
 * @uses Theme_Pagination_HTML
 */
class Theme_Pagination_Post extends Theme_Pagination_HTML {
	...
	public function __construct( $args = array() ) {
		...
		$this->default_args = array(
			...
			'prev_html' => '<a href="{URL}" class="paging-prev">' . esc_html__( '« Previous Entry', 'app' ) . '</a>',
			// modify the text of the next page link
			'next_html' => '<a href="{URL}" class="paging-next">' . esc_html__( 'Next Entry »', 'app' ) . '</a>',
		);

		// register additional tokens
		add_filter( 'theme_pagination_after_setup_item', array( $this, 'add_tokens' ) );
	...
	public function get_pagination_posts() {
		...
		$query = apply_filters( 'theme_pagination_post_pagination_query', $query, $this );
	 ...
	 *
	 * @param Theme_Pagination_Item $item The item to add tokens to.
	 */
	public function add_tokens( Theme_Pagination_Item $item ) {
		if ( ! ( $item instanceof Theme_Pagination_Item_Page ) ) {
			return;
		}
	 ...
	 */
	public function get_post_title( Theme_Pagination_Item $item ) {
		if ( ! ( $item instanceof Theme_Pagination_Item_Page ) ) {
			return;
		}
		...
		$title = apply_filters( 'the_title', $title, $pages[ $page_number ] );
