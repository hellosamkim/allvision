<?php
/**
 * The Theme Pagination presenter.
 * Handles rendering and displaying of a particular pagination.
 *
 * @abstract
 */
class Theme_Pagination_Presenter {

	/**
	 * @var Theme_Pagination
	 * 
	 ...
	 *
	 * @param Theme_Pagination_HTML $pagination Pagination object that will be displayed.
	 */
	public function __construct( Theme_Pagination_HTML $pagination ) {
	 ...
	 *
	 * @param Theme_Pagination_HTML $pagination The pagination object.
	 */
	...
	public function verify_pagination() {
		...
		if ( ! class_exists( $this->get_pagination()->get_collection() ) ) {
			return new WP_Error( 'theme_pagination_unexisting_pagination_collection', __( 'Unexisting pagination collection class.', 'theme_pagination' ) );
		}

		// handle unexisting pagination renderer classes
		if ( ! class_exists( $this->get_pagination()->get_renderer() ) ) {
			return new WP_Error( 'theme_pagination_unexisting_pagination_renderer', __( 'Unexisting pagination renderer class.', 'theme_pagination' ) );
		}
	 ...
	 * @see Theme_Pagination::__construct()
	 */
	public static function display( $pagination, $args = array(), $echo = true ) {
		$pagination_classname = 'Theme_Pagination_' . $pagination;

		// handle unexisting pagination types
		if ( ! class_exists( $pagination_classname ) ) {
			return new WP_Error( 'theme_pagination_unexisting_pagination_type', __( 'Unexisting pagination type class.', 'theme_pagination' ) );
		}
