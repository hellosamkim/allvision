<?php
/**
 * The Theme Pagination renderer class.
 * Handles rendering of the pagination.
 */
class Theme_Pagination_Renderer {

	/**
	 * @var Theme_Pagination_Collection
	 * 
	 ...
	 *
	 * @param Theme_Pagination_Collection $collection Pagination collection object.
	 */
	public function __construct( Theme_Pagination_Collection $collection ) {
	 ...
	 *
	 * @return Theme_Pagination_Collection $collection The collection object.
	 */
	 ...
	 *
	 * @param Theme_Pagination_Collection $collection The new collection object.
	 */
	public function set_collection( Theme_Pagination_Collection $collection ) {
	...
	public function prepare_items( $items = array() ) {
		...
		// allow only Theme_Pagination_Item instances here
		foreach ( $items as $item ) {
			if ( $item instanceof Theme_Pagination_Item ) {
				$ready_items[] = $item;
			}
		}
	...
	public function render( $items = array(), $echo = true ) {
		...
		$items = apply_filters( 'theme_pagination_items_before_render', $items, $this );
		...
		$output = apply_filters( 'theme_pagination_renderer_output', $output, $this );
	 ...
	 *
	 * @param Theme_Pagination_Item $item Item to render.
	 * @return string $output The HTML of the item.
	 */
	public function render_item( Theme_Pagination_Item $item ) {
		// allow item to be modified before setup
		do_action( 'theme_pagination_before_setup_item', $item );

		// setup the item
		$item->setup();

		// allow item to be modified after setup
		do_action( 'theme_pagination_after_setup_item', $item );

		// render the item
		$html = $this->parse_tokens( $item->render(), $item->get_tokens() );

		// allow item HTML to be filtered
		$html = apply_filters( 'theme_pagination_render_item_html', $html, $item );
