 ...
 * @return string
 */
function app_shortalize($input, $words_limit=15, $end='...') {
 ...
 */
function app_taxonomy_ancestor($term_id, $taxonomy) {
 ...
 */
function app_get_meta($key, $id=null) {
	if (!isset($id)) {
		global $post;
		if (empty($post->ID)) {
			return null;
 ...
 * @return array
 */
function app_get_content_by_meta_key($meta_key) {
	global $wpdb;
	$result = $wpdb->get_col('
...
function app_get_page_context() {
	...
	return apply_filters('app_get_page_context', $page_ID);
}
 ...
 */
function app_strip_url_protocol($url) {
 ...
 * @return bool
 */
function app_is_external_url($src, $home_url) {
	$separator = '~';
 ...
 *
 * @see app_strip_url_protocol()
 * @see app_is_external_url()
 *
 ...
 */
function app_generate_file_version($src) {
	# Normalize both URLs in order to avoid problems with http, https
	# and protocol-less cases
	$src = app_strip_url_protocol($src);
	$home_url = app_strip_url_protocol( site_url('/') );

	# Default version
	$version = false;

	if ( !app_is_external_url($src, $home_url) ) {
 ...
 */
function app_enqueue_script($handle, $src, $dependencies=array(), $in_footer=false) {
	wp_enqueue_script($handle, $src, $dependencies, app_generate_file_version($src), $in_footer);
}
 ...
 */
function app_enqueue_style($handle, $src, $dependencies=array(), $media='all') {
	wp_enqueue_style($handle, $src, $dependencies, app_generate_file_version($src), $media);
}
 ...
 */
add_filter('the_content', 'app_shortcode_empty_paragraph_fix');
add_filter('app_content', 'app_shortcode_empty_paragraph_fix', 11);
function app_shortcode_empty_paragraph_fix($content) {
	$array = array(
		'<p>['    => '[',
		']</p>'   => ']',
		']<br />' => ']',
 ...
 */
add_action('wp_head', 'app_display_favicon', 5);
add_action('login_head', 'app_display_favicon', 5);
add_action('admin_head', 'app_display_favicon', 5);
function app_display_favicon() {
	...
	$favicon_uri = apply_filters( 'app_theme_favicon_uri', $theme_uri . '/images/favicon.ico' );

	# Determine version based on file modified time.
	# If the $version is false, the file does not exist
	$version = app_generate_file_version($favicon_uri);
 ...
 */
add_filter('bloginfo', 'app_filter_default_bloginfo_description', 10, 2);
function app_filter_default_bloginfo_description($output, $show) {
	if ($show !== 'description') {
		return $output;
	}
 ...
 */
function app_request_param($key = '') {
	$value = false;
	if (!$key) {
		return $value;
	}
 ...
 */
function app_dynamic_sidebar($index = 1, $options = array()) {
	global $wp_registered_sidebars;
 ...
 */
function app_require_login($redirect = '') {
 ...
 */
function app_require_user_level($level, $redirect = '') {
	$u = wp_get_current_user();
	if ( empty($u->ID) || !( app_user_is($u->ID, 'administrator') && app_user_is($u->ID, $level) ) ) {
		$redirect = ($redirect) ? $redirect : home_url('/');
		wp_redirect($redirect);
		exit;
	}
...
/**
 * Attach all Hooks from `the_content` on `app_content`.
 */
add_filter( 'app_content', 'wptexturize'                          );
add_filter( 'app_content', 'wpautop'                              );
add_filter( 'app_content', 'shortcode_unautop'                    );
add_filter( 'app_content', 'prepend_attachment'                   );
add_filter( 'app_content', 'wp_make_content_images_responsive'    );
add_filter( 'app_content', 'do_shortcode',                     12 );
 ...
 */
function app_attach_embed_filters() {
	global $wp_embed;
	...
	add_filter( 'app_content', [ $wp_embed, 'autoembed' ] );
	add_filter( 'app_content', [ $wp_embed, 'run_shortcode' ] );
}

app_attach_embed_filters();
