 ...
 * Example usage:
 * - Theme_Fragment::create('layout-block-left', array('name'=>'John Doe'))->render();
 * - Theme_Fragment::create(array('layout-block-left', 'layout-block'))->cache(60*60*24)->render();
 */
class Theme_Fragment {
	/**
	...
	function __construct($fragments, $context=array()) {
		...
		if (!$this->template_file) {
			throw new Theme_Fragment_Exception('Crabon Fragment could not locate template for: [' . implode(', ', $fragments) . ']', 1);
		}

		if (is_array($context)) {
			$this->context = $context;
		} else {
			throw new Theme_Fragment_Exception('Crabon Fragment context should be an array of key-value pairs to pass to the fragment. Received ' . gettype($context) . ' instead', 2);
		}
	}
...
class Theme_Fragment_Exception extends Exception {}
